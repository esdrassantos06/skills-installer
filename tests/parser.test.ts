...
describe("parseLine", () => {
  ...
  it('strips leading "npx -y skills add"', () => {
    const r = parseLine("npx -y skills add foo/bar@baz", baseOpts)!;
  ...
  it("handles GitHub URLs", () => {
    const r = parseLine("https://github.com/vercel-labs/skills", baseOpts)!;
...
describe("detectAlreadyInstalled", () => {
  ...
  it('detects "already exists"', () => {
    expect(detectAlreadyInstalled("Target already exists, skipping")).toBe(
      true,
    );
  });
