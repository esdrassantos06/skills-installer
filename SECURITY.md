...
Don't open a public issue. Email the maintainer instead (or use GitHub's "Report a vulnerability" button on the Security tab). I'll respond within 7 days.
...
If you don't hear back in a week, ping me again — assume the email got lost, not ignored.
...
- **Shell injection via input.** `spawn()` is invoked with `shell: false` and an explicit argv. User input is tokenized in `parseLine()` and passed as separate arguments to npx. There's no `eval`, no `exec`, no shell interpolation.
- **Renderer → main IPC.** `contextIsolation: true` and `nodeIntegration: false`. The renderer can only call channels explicitly exposed via `contextBridge`. The main process validates the channel and re-parses inputs.
- **External navigation.** `setWindowOpenHandler` denies in-app navigation and routes external URLs through `shell.openExternal`.
- **Network.** The main process makes exactly one outbound call: a GET to `https://www.skills.sh/api/search`. The URL is hardcoded.
...
- **Supply chain.** `npx skills add` downloads from GitHub. If a repo is compromised between when you read the README and when you install, you'll get the compromised version. Pin to commits/tags for security-sensitive flows.
...
- Treat localStorage values as non-secret. The app stores the agent list and last input there — never paste API keys or tokens into the Installer textarea.
...
Once a fix is ready I'll cut a patch release, then disclose 7 days later via a GitHub Security Advisory. Critical bugs get same-day patches.
